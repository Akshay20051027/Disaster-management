...
function SchoolDashboard() {
    ...
    const fetchDashboard = async () => {
      try {
        const response = await axios.get(
          `http://localhost:8000/api/schools/dashboard/${id}`, // 👈 use id
  ...
  const handleUpload = async () => {
    ...
    try {
      await axios.post(
        `http://localhost:8000/api/students/bulk-upload?schoolId=${id}`,
        formData,
        {
          headers: {
      ...
      const refreshed = await axios.get(
        `http://localhost:8000/api/schools/dashboard/${id}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleDownload = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8000/api/students/download/${id}`,
        { responseType: "blob" }
      );
